/** Grid-aligned rectangle in cell coordinates (matches backend `grid-rect.ts`). */
...
export function isValidMovePosition(
  ...
  if (!rectWithinGarden(rect, gardenWidth, gardenHeight)) return false;
  for (const a of all) {
    if (a.id === movingId) continue;
    if (gridRectsOverlap(rect, a)) return false;
  }
  return true;
}
